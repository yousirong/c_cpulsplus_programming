...
{                   
    ...
    if (elec_month>1 && elec_month<=12)   //  전기 사용한 달이 1~12월 인 경우
    {
        switch (elec_month)  // 전력 수요 성수기 동 하계 계산
        {
        case 2:
        case 7:
        case 8:
        ...
        case 12:
            ...
            if (elec_use_sum > 0 && elec_use_sum <= 200)  // 200kWh 이하 사용
            {
                normal_won = 910;
                use_won = 93.3 * elec_use_sum;
            }
            else if (elec_use_sum > 201 && elec_use_sum <= 400) // 200kWh ~ 400kWh 사용
            {
                normal_won = 1600;
                use_won = 93.3 * 200 + 187.9 * (200 - elec_use_sum);
            }
            else if (elec_use_sum > 400 && elec_use_sum<=1000) // 400kWh~ 1000kWh 사용
            {
                normal_won = 7300;
                use_won = 93.3 * 200 + 187.9 * 200 + 280.6 * (400 - elec_use_sum);
            }
            else if (elec_use_sum > 1000)   // 성수기 동 하계 1000kWh 초과
            { // 하계, 동계 1000kwh 초과
                normal_won = 7300;
        ...
        default:   // 성수기 동하계 아닌 경우
            ...
            else if (elec_use_sum > 400)  // 400kWh 이상 사용
            {
                normal_won = 7300;
...
/*
문제의 정의 : 
  이 문제는 switch 함수와 조건문을 적절하게 사용하여 성수인 동하계인 것과 비성수기로 나누어서 요금 계산하는 문제입니다.
