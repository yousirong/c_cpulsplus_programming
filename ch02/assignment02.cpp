...
int julian_cal_func(int y, int m, int d)  // 줄리안 계산 함수
{
	y += 8000;
	if (m < 3)
	{
...
int result_func(int y, int m, int d)  // 잘못입력될경우 오류 문장 출력과 날짜의 요일을 구하는 함수
{
	...
	if (y % 4 == 0 && y % 100 != 0 || y % 400 == 0) { // 윤년인 경우 
		if (m == 2 && d > 29 && d < 1) { //윤년인데 2월달 29일 넘김 일 수 가 틀린 경우
			cout << "Wrong day! Try again!" << endl;
			return -1;
		}
		else if (m==1 || m == 3 || m == 7 ||m == 8 ||m == 10 ||m == 12) { //2월 달이 아닌 마지막 날짜가 31일인 달에서 일 수 가 초과된경우 
				if (d < 1 || d>31) {
					cout << "Wrong day! Try again!" << endl;
					return -1;
				...
				} else {  // 2월 달이 아닌 마지막 날짜가 30일인 달에서 일 수 가 초과된 경우
				if (d < 1 || d>30) {
					cout << "Wrong day! Try again!" << endl;
					return -1;
			} 
		...
		if (m == 2 && d > 28 && d < 1) {  // 윤년이 아닌데 28일 초과 된 경우
			cout << "Wrong day! Try again!" << endl;
			return -1;
		}
		else if (m==1 || m == 3 || m == 7 ||m == 8 ||m == 10 ||m == 12) {//2월 달이 아닌 마지막 날짜가 31일인 달에서 일 수 가 초과된경우 
				...
				if (d < 1 || d>30) { // 2월 달이 아닌 마지막 날짜가 30일인 달에서 일 수 가 초과된 경우
					cout << "Wrong day! Try again!" << endl;
					return -1;
			} 
			}
...
/*
문제의 정의 : 
  이 문제는 줄리안 계산 해주는 함수와 오류문장출력과 함께 해당날짜 요일을 구해주는 함수를 만들어서 풀이하는 문제입니다.
문제의 해결 방안 : 
		...
		달력에는 윤년도 있지만 30일인 달이 있고, 31일인 달이 있으므로 if구문을 통해 구분해서 오류를 출력하도록 작성했습니다. 
