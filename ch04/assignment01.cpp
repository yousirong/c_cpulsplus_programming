...
/*
문제의 정의:
분수는 "분자|분모" 형태로 주어지는데 주어진 문자열에서 두 분수와 연산자를 추출하여, 해당 분수에 대한 사칙연산을 수행한 후, 결과를 약분된 형태로 출력하는 프로그램을 작성하는 것입니다. 

문제의 해결 방안:
1. 사용자로부터 입력 문자열을 받음.
2. 입력 문자열에서 첫 번째 분수의 분자와 분모를 추출
3. 입력 문자열에서 연산자를 추출
4. 입력 문자열에서 두 번째 분수의 분자와 분모를 추출
5. 입력된 분자와 분모를 사용하여 `Arithmetic` 클래스의 객체를 생성
6. 연산자에 따라 두 분수를 덧셈, 뺄셈, 곱셈 또는 나눗셈을 수행
7. 계산 결과는 약분하여 출력

위의 코드에서, `Arithmetic` 클래스는 주어진 분자와 분모를 만들고, 최대공약수와 최소공배수를 계산
클래스를 사용하여 분수의 사칙연산을 수행합니다. 또한, 분모가 0인 경우 예외처리가 수행

또한, 코드에서는 입력 문자열을 처리하기 위해 STL의 문자열 함수를 사용
문자열에서 필요한 정보를 추출하여 `Arithmetic` 클래스의 객체로 변환하고, 계산 결과를 출력
*/
